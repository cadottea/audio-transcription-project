...
# Flag to enable/disable ANSI color coding
...
def run_whisper(audio_file, output_dir, model):
    ...
    try:
        ...
        # Strip ANSI color codes if coloring is disabled
        if not ENABLE_COLOR_CODING:
            import re
            ansi_escape = re.compile(r'(?:\x1B[@-_]|[\x1B\x9B][0-?]*[ -/]*[@-~])')
            colored_output = ansi_escape.sub('', colored_output)
...
# Function to call the keyword analyzer
def run_keyword_analyzer(output_dir, flagged_file):
    """Call the keyword analyzer program to scan the transcriptions."""
    analyzer_script = os.path.join(os.path.dirname(__file__), "keyword_analyzer.py")
    try:
        print(f"Running Keyword Analyzer on output directory: {output_dir}")
        result = subprocess.run(
            ["python", analyzer_script, output_dir, flagged_file],
            check=True,
            capture_output=True,
            text=True
        )
        print("Keyword Analyzer Output:\n", result.stdout)
    except subprocess.CalledProcessError as e:
        print(f"Error: Keyword Analyzer failed with return code {e.returncode}")
...
def main():
    ...
    flagged_file = os.path.join(output_dir, "flagged.txt")  # File to store flagged keywords
    ...
    # Run the keyword analyzer on the transcriptions
    run_keyword_analyzer(output_dir, flagged_file)
