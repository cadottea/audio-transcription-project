...
def split_audio(input_file, transcribe_dir, segment_duration=5, overlap_duration=1):
    # Get the duration of the audio file
    probe = ffmpeg.probe(input_file, v="error", select_streams="a", show_entries="stream=duration")
    duration = float(probe['streams'][0]['duration'])

    # Calculate the number of parts based on the segment duration (with overlap)
    num_parts = int((duration - overlap_duration) // (segment_duration - overlap_duration))
    ...
    # Split the audio file into parts with overlap
