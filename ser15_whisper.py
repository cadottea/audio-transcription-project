...
def preprocess_audio(audio_path, feature_extractor, save_preprocessed_audio=True):
...
# Predict emotion
def predict_emotion(audio_path, model, feature_extractor, id2label):
    # Preprocess the audio and save the preprocessed version for verification
    inputs = preprocess_audio(audio_path, feature_extractor, save_preprocessed_audio=True)
...
def run_whisper_with_emotion(audio_file, output_file_path, model, segment_duration):
    ...
    command = [
        ...
        "--max-len", str(segment_duration), "--word-thold", "0.5",
    ]
